...
#define CONFIG_EXTRA_SETTINGS \
    ...
    #define CONFIG_BOOTCOMMAND "mmc rescan;setenv setargs setenv bootargs ${bootargs}; run setargs; mmc read ${baseaddr} 0x1800 0x3000; bootm ${baseaddr}"
#endif
#ifdef CONFIG_SFC_NOR
    #define CONFIG_BOOTCOMMAND "sf probe;setenv setargs setenv bootargs ${bootargs}; run setargs; sf read ${baseaddr} 0x50000 \\${kern_len}; bootm ${baseaddr}"
#endif
#ifdef CONFIG_SFC_NAND
    #define CONFIG_BOOTCOMMAND "sf probe;setenv setargs setenv bootargs ${bootargs}; run setargs; sfcnand read 0x50000 0x200000 ${baseaddr} ; bootm ${baseaddr}"
#endif

#define CONFIG_BOOTARGS "mem=\${osmem} rmem=\${rmem} console=\${serialport},\${baudrate}n8 panic=\${panic_timeout} root=/dev/mtdblock3 rootfstype=squashfs init=/init mtdparts=jz_sfc:256k(boot),64k(env),\\${kern_size}(kernel),\\${rootfs_size}(rootfs),-(rootfs_data)"
